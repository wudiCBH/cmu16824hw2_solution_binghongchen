...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        C = Cr2 // r2
        x = x.view(B, r2, C, hr, wr) # ( batch, downscale_factor^2, channel, height, width)
        x = x.permute(1, 0 ,2, 3, 4) # (downscale_factor^2, batch, channel, height, width)
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # to the layer output.
        ##################################################################
        x = self.upsample_residual(x) + self.layers(x)
...
class Generator(torch.jit.ScriptModule):
    ...
    def __init__(self, starting_image_size=4):
        ...
        self.out_features = self.in_features * self.starting_image_size * self.starting_image_size # 2048
        self.dense = nn.Linear(in_features=self.input_channels, out_features=self.out_features)
        self.layers = nn.Sequential(
            ResBlockUp(input_channels=self.input_channels, kernel_size=3, n_filters=self.n_filters),
            ResBlockUp(input_channels=self.n_filters, kernel_size=3, n_filters=self.n_filters),
            ResBlockUp(input_channels=self.n_filters, kernel_size=3, n_filters=self.n_filters),
            nn.BatchNorm2d(self.n_filters, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.ReLU(),
            nn.Conv2d(self.n_filters, 3, kernel_size=3, stride=1, padding=1),
            nn.Tanh(),
        )
        ##################################################################
        #                          END OF YOUR CODE                      #
    ...
    def forward_given_samples(self, z):
        ...
        ##################################################################
        image = self.dense(z) #(batch size, 2048)
        image = image.view(-1, self.in_features, self.starting_image_size, self.starting_image_size) #(batch size, 128, 4, 4)
        image = self.layers(image) #(batch size, 3, 32, 32)
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def __init__(self):
        ...
        self.n_filters = 128
        self.layers = nn.Sequential(
            ResBlockDown(input_channels=3, kernel_size=3, n_filters=self.n_filters),
            ResBlockDown(input_channels=self.n_filters, kernel_size=3, n_filters=self.n_filters),
            ResBlock(input_channels=self.n_filters, kernel_size=3, n_filters=self.n_filters),
            ResBlock(input_channels=self.n_filters, kernel_size=3, n_filters=self.n_filters),
            nn.ReLU(),
        )
        self.dense = nn.Linear(in_features=self.in_features, out_features=1, bias=True)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
